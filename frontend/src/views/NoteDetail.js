...
function NoteDetail() {
  ...
  useEffect(() => {
    if (authTokens) {
      axiosInstance.get(`http://shiv_backend_1:8000/api/notes/${id}/`).then((response) => {
  ...
  const handleUpdate = async (formData) => {
    try {
      const response = await axiosInstance.patch(
        `http://shiv_backend_1:8000/api/notes/${id}/`,
        formData
      );
      setNote(response.data);
      setEditing(false);
  ...
  const handleDelete = async () => {
    try {
      await axiosInstance.delete(`http://shiv_backend_1:8000/api/notes/${id}/delete`);
      history.push("/notes");
    } catch (error) {
      console.error(error);
    }
